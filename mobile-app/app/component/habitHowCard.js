...
function HabitHowCard(props) {
    const {habitId, setShowTips} = props
    const [habitHows, setHabitHows] = useState(howListInit[habitId])
    ...
    <View style={{ backgroundColor: "rgba(224, 242, 241,0.8)", flex: 1 }}>
        <View style={containerStyles.habitCardContainer}>
            <View style={{ position: "absolute", left: 300, marginTop:15 }}>
                <TouchableOpacity style={[buttonStyles.button, { paddingHorizontal: 5 }]} onPress={() => closeHabitCard()}>
                    <Text style={textStyles.textSmallRed}>X</Text>
                </TouchableOpacity>
            </View>
            <View style={containerStyles.optionBodymainContainer}>
                <View style={[containerStyles.optionWrapper, {marginTop:30}]}>
                    <Text style={textStyles.textMedBlue}>Tips on how to build the habit</Text>
                </View>
