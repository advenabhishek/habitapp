...
const sendEatingFruitNotification = async (userHabitId, data) => {
    ...
    for (let weekday = 0; weekday < data.length; weekday++) {
        ...
        let timeDiffNotificationMins = Math.floor(timeDiffInHours / amount * 60)
        ...
        for (let i = 0; i < amount; i++) {
            ...
            Notifications.scheduleNotificationAsync({
                content: {
                    title: 'Eat fruit now',
                    body: 'Hi, please eat a fruit now.',
                    categoryIdentifier: 'eatingFruitLog',
...
const handleEatingFruitLogListener = async ({ notification, actionIdentifier, userText }) => {
    switch (actionIdentifier) {
        case 'yes':
            ...
            var logAmount = 1
            analyticsTrack(analytics, "notif:eatingFruitLogYes", { "habitStartTime": new Date(), "amount": logAmount })
            ...
            break
        case 'later':
            analyticsTrack(analytics, "notif:eatingFruitLogLater")
            break
    }
}

export { sendEatingFruitNotification, handleEatingFruitLogListener }